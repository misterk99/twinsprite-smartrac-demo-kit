...
- (IBAction) clickBack {
    ...
    UnitySendMessage([objectName UTF8String], [functionName UTF8String], @"CANCELLED");
...
- (void)viewDidLoad {
	...
	if (cameraInput == nil) {
        // Send result to Unity
        UnitySendMessage([objectName UTF8String], [functionName UTF8String], @"ERROR");
        return;
	}
...
- (void) rotateCallback:(NSNotification *)notification {
    ...
    switch (orientation) {
        ...
        case portrait:
            self.previewContainer.transform = CGAffineTransformMakeRotation(0);
            self.previewContainer.frame = CGRectMake(0, 0, parentViewController.view.frame.size.width,  parentViewController.view.frame.size.height);
            break;
            
        case portraitUpsideDown:
            self.previewContainer.transform = CGAffineTransformMakeRotation(M_PI);
            self.previewContainer.frame = CGRectMake(0, 0, parentViewController.view.frame.size.width,  parentViewController.view.frame.size.height);
